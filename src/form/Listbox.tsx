...
/** Gestyltes Listbox-Dropdown (Headless UI) mit Label, Platzhalter und Mehrfachauswahl. */
export function Listbox<T>({
  ...
  return (
    <div className="flex flex-col gap-2">
      {label && <label className="block text-sm/6 font-medium text-gray-900 dark:text-gray-300">{label} {optional && <span className="text-gray-400 text-xs">{`OPTIONAL`}</span>}{required && <span className="text-red-500 pl-0.5 relative -top-0.5">*</span>}</label>}
      <Headless.Listbox {...props} multiple={multiple}>
        ...
        >
          ...
          <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
            <svg
              className="h-5 w-5 stroke-zinc-500 group-data-[disabled]:stroke-zinc-600 sm:h-4 sm:w-4 dark:stroke-zinc-400 forced-colors:stroke-[CanvasText]"
              viewBox="0 0 16 16"
              aria-hidden="true"
              fill="none"
            >
...
export type ListboxOptionProps<T> = { className?: string; children?: React.ReactNode } & Omit<
  Headless.ListboxOptionProps<'div', T>,
  'as' | 'className'
>

/** Einzelne Option innerhalb der Listbox (mit Häkchen für den ausgewählten Eintrag). */
export function ListboxOption<T>({
  ...
  const sharedClasses = clsx(
    ...
    // Icons
    '[&>[data-slot=icon]]:w-5 [&>[data-slot=icon]]:h-5 [&>[data-slot=icon]]:shrink-0 sm:[&>[data-slot=icon]]:size-4',
    ...
    <Headless.ListboxOption as={Fragment} {...props}>
      {({ selectedOption }) => {
        ...
        return (
          ...
          >
            <svg
              className="relative hidden w-5 h-5 self-center stroke-current group-data-[selected]/option:inline sm:w-5 sm:h-5"
              viewBox="0 0 16 16"
              fill="none"
              aria-hidden="true"
            >
